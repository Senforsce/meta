...
func helpPrinter(_ kong.HelpOptions, ctx *kong.Context) error {
	...
	fmt.Println()
	fmt.Println("  The Hub for managing metarepos by Senforsce")
	...
	fmt.Println(
		codeBlockStyle.Render(
			lipgloss.JoinVertical(
				lipgloss.Top,
				lipgloss.JoinHorizontal(lipgloss.Left, programStyle.Render("tndr"), argumentStyle.Render("init"), flagStyle.Render("[-i report.ttl] [--interactive]")),
				lipgloss.JoinHorizontal(lipgloss.Left, programStyle.Render("tndr"), argumentStyle.Render("--execute"), stringStyle.Render("\"git fetch origin\""), flagStyle.Render("[--flags]   ")),
			),
		),
	)
